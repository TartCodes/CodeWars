...
function slicesToName(n) { 
  ...
  let arr = new Array(Math.floor(n/2)).fill('sandwich')
  if(n % 2 === 1) arr.unshift('bread');            
  return arr.join(' ')
  console.log(n)
} 

function nameToSlices(n) { 
  if(typeof(n) !== 'string' || n === '' || n === 'bread' || n.lastIndexOf === -1 || n.lastIndexOf('bread') > 0){return null}
   n = n.split(' ')
  let test = n.map(e => e === 'sandwich' ? true : e === 'bread' ? true : false)
  let bread = n[0] === 'bread'
  return test.includes(false) ? null : bread ? n.length * 2 - 1 : n.length * 2
}
 
// have fun! what a mess babyyyyy