...
function getTheVowels(word) {
    let vowels = 'aeiou', //declare vowels
        vi = 0, //declare index of vowels 
        result = 0 // counter
     for(let i = 0; i < word.length; i++){
       if(word[i] === vowels[vi]){ //if word[i] = vi, which starts at 0 or a in this case, countinue the count
           result++
         if(vi === 4){ //however if vi = 4, that would me u, so need to reset the counter
           vi = 0 //reset
         } else { //otherswise go to the next letter
           vi++
         }
       }
         }
